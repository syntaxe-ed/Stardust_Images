	...
	async photosList() {
		...
		await Promise.all(this.state.photos.map(currentPhoto => {
			if (currentPhoto.galleryTitle === title) {
				i++;
				if (photos.length === 0){
					return <GalleryCard small={12} large={3} items={this.state.photos} photo={currentPhoto.fileName} text={currentPhoto.fileName} folder={this.state.fileName + '/'} key={currentPhoto._id}/>
				}
